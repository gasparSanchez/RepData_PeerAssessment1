...
levels(dataImp$weekday) <-  c("weekend",
                              rep("weekday",4),
                              "weekend",
                              "weekday")
averageStepsImp.weekday <- aggregate(dataImp[which(dataImp$weekday=="weekday"),]$steps, list(dataImp[which(dataImp$weekday=="weekday"),]$interval), FUN=mean, na.rm=TRUE)
averageStepsImp.weekday$weekday <- "weekday"
averageStepsImp.weekend <- aggregate(dataImp[which(dataImp$weekday=="weekend"),]$steps, list(dataImp[which(dataImp$weekday=="weekend"),]$interval), FUN=mean, na.rm=TRUE)
...
library(ggplot2)
ggplot(data=averageStepsImp, aes(x=Group.1, y=x)) + geom_line() +
 facet_grid(.~weekday)
```
